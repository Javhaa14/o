      ...
      <div className="max-w-4xl mx-auto">
        <div className="backdrop-blur-xl bg-white/70 border border-white/40 shadow-2xl rounded-3xl p-8 mb-8">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-4 mb-6 border border-blue-200">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-blue-600">🗄️ MongoDB Atlas</h3>
              ...
              <div className="text-right">
                ...
                <p className="text-blue-600 text-xs mt-1">valentine-app.dates</p>
              </div>
            </div>
          </div>
          ...
          <div className="text-center mb-6">
            <p className="text-lg text-rose-700">
              Нийт болзоо: <span className="font-bold text-2xl text-rose-600">{dates.length}</span>
            </p>
          </div>
        </div>
        ...
        {dates.length === 0 ? (
          <div className="backdrop-blur-xl bg-white/70 border border-white/40 shadow-2xl rounded-3xl p-8 text-center">
            <p className="text-rose-600 text-lg">Одоогоор болзоо байхгүй байна.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {dates.map((date) => (
              <div key={date.id} className="backdrop-blur-xl bg-white/70 border border-white/40 shadow-2xl rounded-3xl p-6">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
                  ...
                  <div className="mt-2 lg:mt-0">
                    <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                      ...
                      {date.dateConfirmed ? '✅ Баталгаажсан' : '❌ Баталгаажаагүй'}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4">
                    <h3 className="font-semibold text-blue-600 mb-2">📅 Огноо</h3>
                    <p className="text-blue-700">{date.date} - {date.day}</p>
                  </div>
                  
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4">
                    <h3 className="font-semibold text-green-600 mb-2">🕐 Цаг</h3>
                    <p className="text-green-700">{date.time}</p>
                  </div>
                </div>
                
                <div className="mt-4 bg-gradient-to-r from-purple-50 to-violet-50 rounded-xl p-4">
                  <h3 className="font-semibold text-purple-600 mb-2">🎯 Үйл ажиллагаанууд</h3>
                  <div className="flex flex-wrap gap-2">
                    {date.activities.map((activity, index) => (
                      <span key={index} className="bg-purple-200 text-purple-700 px-3 py-1 rounded-full text-sm">
                        {activity}
                      </span>
                    ))}
                  </div>
